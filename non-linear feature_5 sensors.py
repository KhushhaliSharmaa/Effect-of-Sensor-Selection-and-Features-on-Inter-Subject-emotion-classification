...
for i in range(no_of_users):#nUser  #4, 40, 32, 40, 8064
        ...
        if(i%1 == 0):
            ...
            for k in range(nTrial):
                ...
                C_EEG_Trial = Ctrial[0:5, 384:8064] #3sec baseline removal
                #C_EEG_Trial = Ctrial[0:32, 4224:8064] #3sec baseline removal
                C_Baseline_EEG = Ctrial[0:5, 0:384] #Extracting baseline
                ...
                for K in range(np.shape(C_EEG_Trial[:,1])[0]):
                    # Filtering
                      ...
                      filtered_signal = scipy.signal.lfilter(b_filt, a_filt, C_EEG_Trial[0:5,:], axis = 0)
...
np.save('nonlinear_feat_5sensor',Feature_chunk)
