...
for i in range(no_of_users):#nUser  #4, 40, 32, 40, 8064
        ...
        if(i%1 == 0):
            ...
            for k in range(nTrial):
                ...
                C_EEG_Trial_old = Ctrial[0:15, 384:8064] #3sec baseline removal
                C_EEG_Trial = Ctrial[0:15, 4224:8064] #3sec baseline removal
                C_Baseline_EEG = Ctrial[0:15, 0:384] #Extracting baseline
                ...
                for K in range(np.shape(C_EEG_Trial[:,1])[0]):
                    # Filtering
                      ...
                      filtered_signal = scipy.signal.lfilter(b_filt, a_filt, C_EEG_Trial[0:15,:], axis = 0)
                      ...
                      filtered_theta = scipy.signal.lfilter(b_theta, a_theta, C_EEG_Trial[0:15,:], axis = 0)
                      ...
                      filtered_alpha = scipy.signal.lfilter(b_alpha, a_alpha, C_EEG_Trial[0:15,:], axis = 0)
                      ...
                      filtered_beta = scipy.signal.lfilter(b_beta, a_beta, C_EEG_Trial[0:15,:], axis = 0)
                      ...
                      filtered_gamma = scipy.signal.lfilter(b_gamma, a_gamma, C_EEG_Trial[0:15,:], axis = 0)
...
np.save('all_feat_15sensor.npy',Feature_chunk)