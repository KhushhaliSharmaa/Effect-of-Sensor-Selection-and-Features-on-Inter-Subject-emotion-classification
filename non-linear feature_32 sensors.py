...
for i in range(no_of_users):#nUser  #4, 40, 32, 40, 8064
        ...
        if(i%1 == 0):
            ...
            fname = "C:/Users/lenovo/Documents/Khushali_Project/Database/s"+str(name)+".dat"
            ...
            for k in range(nTrial):
                ...
                #EEG_Sensor = EEG_Trial[[0,7,13,14,15,16,20,25,28,30]]
                Ctrial = EEG_Trial - np.mean(EEG_Trial, axis=0) #Common Average Referencing
                C_EEG_Trial = Ctrial[0:32, 384:8064] #3sec baseline removal
                #C_EEG_Trial = Ctrial[0:32, 4224:8064] #3sec baseline removal
                C_Baseline_EEG = Ctrial[0:32, 0:384] #Extracting baseline
                ...
                for K in range(np.shape(C_EEG_Trial[:,1])[0]):
                    # Filtering
                      ...
                      filtered_signal = scipy.signal.lfilter(b_filt, a_filt, C_EEG_Trial[0:32,:], axis = 0)
                      ...
                      # filtered_theta = scipy.signal.lfilter(b_theta, a_theta, C_EEG_Trial[0:5,:], axis = 0)
                      ...
                      # filtered_alpha = scipy.signal.lfilter(b_alpha, a_alpha, C_EEG_Trial[0:5,:], axis = 0)
                      ...
                      # filtered_beta = scipy.signal.lfilter(b_beta, a_beta, C_EEG_Trial[0:5,:], axis = 0)
                      ...
                      # filtered_gamma = scipy.signal.lfilter(b_gamma, a_gamma, C_EEG_Trial[0:5,:], axis = 0)
...
#%%             #FEATURE EXTRACTION            
                ...
                Feature_array.append([Ent_Value, HFD_Value, Corr_Value])
...
np.save('nonlinear_feat_32sensor',Feature_chunk)
